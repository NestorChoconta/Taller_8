...
public class ProductRepositoryImpl implements Repository<Product>{
    ...
    public List<Product> listAllObj() throws SQLException {
        sql = "select p.product_name, p.product_value, p.category_id " + "from products p order by p.product_name, p.product_value ";
    ...
    public Product byIdObj(Integer id) throws SQLException {
        sql =  sql = "select p.product_id, p.product_name, p.product_value, p.category_id " + "from products p where p.product_id=? ";
    ...
    public Integer saveObj(Product product) throws SQLException {
        ...
        if(product.getProduct_id() != null && product.getProduct_id() > 0) {
            ...
            sql= "insert into products (product_name, product_value, category_id)"+"values (upper(?),?,?)";
        }
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)){
